import numpy as np
from .com_utils import center_of_mass, create_2Dgrid, loc2Dto1D
...
class Environment(object):
    ...
    def build_transition_matrix(self, max_step_size=1):
        ...
        for i in range(self.N_states):
            for j in range(self.N_states):
                T[i, j] = (
                    np.sqrt((self.x_arr[j] - self.x_arr[i]) ** 2 + (self.y_arr[j] - self.y_arr[i]) ** 2)
    ...
    def add_food_patches(self):  # , food_statistics_type="drop_food_once"):
        ...
        for pi in range(N_patches):
            ...
            list_newfood_loc_1d = loc2Dto1D(x_locs.flatten(), y_locs.flatten(), self.edge_size)
