...
from .com_utils import loc1Dto2D, loc2Dto1D
    ...
    def run(self):
        ...
        for ti in range(self.N_frames - 1):
            ...
            for ai, agent in enumerate(self.list_agents):
                ...
                self.calories_acquired_mat[ai, ti] = (delta_food_cal[prev_loc_1d] / self.phi_agents[prev_loc_1d][0])[0]
                ...
                xloc_allagents, yloc_allagents = loc1Dto2D(self.loc_1D_allagents, self.env.edge_size)
                ...
                phi_visible_mat = agent.compute_visible_locations(xloc_self, yloc_self, self.env.edge_size)
                ...
                self.calories_expended_mat[ai, ti] = agent.caloric_cost_per_unit_dist * dist_traveled
                ...
                if len(self.list_agents) > 1:
                    dist_to_neighbors = np.sqrt(
                        (xloc_otheragents - xloc_self) ** 2 + (yloc_otheragents - yloc_self) ** 2
                    )
