...
class Communicators(object):
    ...
    def value_update(self, w_food_self, w_food_others):
        ...
        sum_weighted_features = (
            1 - self.c_trust
        ) * w_food_self + self.c_trust * w_food_others
        value = self.SR @ sum_weighted_features  # (N_states, 1) vector
        return value

    def policy_update(self, prev_loc_1d, value):
        # eligible states are those specified by the transition matrix. Can constrain further
        # to exclude states not occupied by other agents
        ...
        if self.doProbabilisticPolicy:
            if self.doSoftmaxPolicy:
                ...
                # prob_arr = value_eligible - np.min(value_eligible) # shift values
                # so they are all positive and add some noise
