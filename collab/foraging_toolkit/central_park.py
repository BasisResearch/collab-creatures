...
def cp_generate_visibility(
    ...
    for bird in range(len(birds)):
        ...
        for frame in times_b:
            ...
            range_df["visibility"] = range_df["distance"].apply(lambda d: visibility_vs_distance(d, visibility_range))
        ...
        for frame in times_b:
            ...
            if visible_birds:
                ...
                proximity[bird][frame_index]["proximity"] += [
                    ...
                    for s in np.sqrt(
                        (proximity[bird][frame_index]["x"] - o_x) ** 2 + (proximity[bird][frame_index]["y"] - o_y) ** 2
                    )
                ]
        ...
        for frame in times_b:
            ...
            if visible_birds:
                for vb in visible_birds:
                    ...
                    o_x = birds[vb_loc].loc[birds[vb_loc]["time"] == frame, "x"].values[0]
                    o_y = birds[vb_loc].loc[birds[vb_loc]["time"] == frame, "y"].values[0]
    ...
    for bird in range(sim.num_birds):
        ...
        for frame in range(len(df) - 1):
            ...
            _hf["how_far_squared_scaled"] = (
                -_hf["how_far_squared"] / (2 * (sim.step_size_max + visibility_range) ** 2) + 1
            )
