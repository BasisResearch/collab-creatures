...
from collab.foraging.toolkit.proximity import (
    foragers_to_forager_distances,
    proximity_score,
)
from collab.foraging.toolkit.utils import generate_grid
from collab.foraging.toolkit.visibility import visibility_vs_distance
...
def cp_generate_proximity_score(
    ...
    foragers = obj.foragers.copy()
    foragersDF = obj.foragersDF.copy()

    if end is None:
        end = len(foragers[0])

    if forager_distances is None:
        forager_distances = foragers_to_forager_distances(obj)
        ...
        for frame in times_b:
            ...
            if visible_foragers:
                for vb in visible_foragers:
                    o_x = (
                        foragers[vb - 1]
                        .loc[foragers[vb - 1]["time"] == frame, "x"]
                        .values[0]
                    )
                    o_y = (
                        foragers[vb - 1]
                        .loc[foragers[vb - 1]["time"] == frame, "y"]
                        .values[0]
                    )
        ...
        for frame in times_b:
            ...
            if visible_foragers:
                for vb in visible_foragers:
                    ...
                    o_x = (
                        foragers[vb_loc]
                        .loc[foragers[vb_loc]["time"] == frame, "x"]
                        .values[0]
                    )
                    o_y = (
                        foragers[vb_loc]
                        .loc[foragers[vb_loc]["time"] == frame, "y"]
                        .values[0]
                    )
...
def cp_derive(obj, optimal_list, distances=None):
    objects = {}
...
def cp_derive_sps(obj, optimal_list, distances=None):
    objects = {}
