...
class Fish_IndependentRates:
    def __init__(self, N : Optional[int] = 4, arena_size : Optional[float] = 50, Tmax : Optional[float] = 10, dt : Optional[float] = 0.1, random_state : Optional[int] = 0, vicsek_params : Optional[dict[str,Any]] = None, pairwiseCopying_params : Optional[dict[str,Any]] = None, diffusion_params : Optional[dict[str,Any]] = None):
        """
        ...
        self.velocities[:,0,1] = 2*np.pi * self.rng.random(N) - np.pi #heading angles in [-np.pi, np.pi)
    ...
    def evolve_positions(self, t_ind, nSteps):
        ...
        for i in range(nSteps):
            ...
            self.trajectories[:,t_ind+1,0] = self.trajectories[:,t_ind,0] + vx * self.dt
            self.trajectories[:,t_ind+1,1] = self.trajectories[:,t_ind,1] + vy * self.dt
