...
def _generate_combined_DF(
    ...
    """
    list_DFs = [
        _generate_DF_from_nestedList(p) for p in predictors_and_scores.values()
    ]
    combinedDF = list_DFs[0]

    for i in range(1, len(list_DFs)):
        combinedDF = combinedDF.merge(
            list_DFs[i], how="inner"
        )
    ...
    if add_scaled_values:
        for key in predictors_and_scores.keys():
            ...
            combinedDF[f"{key}_scaled"] = (
                combinedDF[key] - column_min
...
def derive_predictors_and_scores(
    ...
    """
    A function that calculates a chosen set of predictors and scores for data by inferring their names from keys in `predictor_kwargs` & `score_kwargs`,
    ...
    for score_name in score_kwargs.keys():
        ...
        generate_function = getattr(ftk, function_name)
        derived_quantities[score_name] = generate_function(
            foragers_object, score_name
        )
    ...
    derivedDF = _generate_combined_DF(
        derived_quantities, dropna, add_scaled_values
    )
