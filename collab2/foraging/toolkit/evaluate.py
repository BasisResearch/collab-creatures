...
def evaluate_performance(model, guide, predictors, outcome, num_samples = 1000, plot = True):

    
    predictive = Predictive(model = model, guide = guide, num_samples =1000, parallel = True)
    ...
    coverage = ((predictions_lower <= outcome[next(iter(outcome.keys()))]) & (outcome[next(iter(outcome.keys()))] <= predictions_upper)).float().mean()
    ...
    rsquared = 1 - (torch.sum(residuals ** 2) / torch.sum((outcome[next(iter(outcome.keys()))] - observed_mean) ** 2))
    ...
    if plot:
        ...
        axs[0].set_title("Ratio of outcomes within 95% CI: {:.2f}".format(coverage.item()))
