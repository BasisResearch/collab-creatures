...
def filter_by_distance(foragersDF : pd.DataFrame, f : int, t : int, interaction_length : float, interaction_constraint : Callable[[List[int],int,int,pd.DataFrame,Any],List[int]] = None, interaction_constraint_params : dict = None)->List[int]:
    ...
    foragers_ind = positions.loc[positions["distance"]<=interaction_length, "forager"].tolist()

    if interaction_constraint is not None:
        foragers_ind = interaction_constraint(foragers_ind, f, t,foragersDF,  **interaction_constraint_params)
    return foragers_ind