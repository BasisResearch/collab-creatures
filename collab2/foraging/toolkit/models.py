...
def add_linear_heteroskedastic_component(
    ...
    sigma_prediction_child = continuous_contribution(
        child_continuous_parents, f"{child_name}_sigma", sigma_leeway,
    )
    
    #sigma_prediction_child_transformed = (torch.abs(sigma_prediction_child - sigma_prediction_child.mean()) + 0.01)
    
    sigma_prediction_child_transformed = torch.nn.functional.softplus(sigma_prediction_child)
    ...
    with data_plate:
        ...
        child_observed = pyro.sample(  # type: ignore
            f"{child_name}",
            dist.Normal(mean_prediction_child, sigma_prediction_child_transformed),
            obs=observations,
        )
...
class HeteroskedasticLinear(pyro.nn.PyroModule):
    ...
    def __init__(self, predictors: Dict[str, torch.Tensor], outcome: Dict[str,torch.Tensor],
                  leeway=.6, sigma_leeway = .2):
        super().__init__()
