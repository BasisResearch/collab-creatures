...
class Environment(object):
    def __init__(
        ...
        self.x_arr, self.y_arr, self.locs_1d_arr = util.create_2Dgrid(
            edge_size
        )
    ...
    def add_food_patches(self, food_statistics_type="drop_food_once"):
        ...
        N_patches = np.ceil(
            self.N_total_food_units / N_units_per_patch
        ).astype(int)
        ...
        if food_statistics_type == "drop_food_once":
            for pi in range(N_patches):
                ...
                x_range, y_range = np.arange(
                    x_start, x_start + self.patch_dim
                ), np.arange(y_start, y_start + self.patch_dim)
...
class BirdAgent(object):
    def __init__(
        ...
        self.SR = np.linalg.pinv(
            np.eye(env.N_states) - discount_factor * env.T_prob
        ...
        self.energy_trajectory = np.zeros(
            [N_timesteps]
        )  # not sure if we need this to be inside the class
        self.energy_trajectory[0] = energy_init
        self.times_at_food = (
            []
        )  # list of frames the agent is at a food location
    ...
    def compute_visible_locations(self, xloc_self, yloc_self, env):
        ...
        phi_visible_mat = (
            np.sqrt((x_grid - xloc_self) ** 2 + (y_grid - yloc_self) ** 2)
            <= self.sight_radius
        )
...
class SimpleAgent(object):
    def __init__(
        ...
        self.discount_factor = discount_factor  # scalar between 0 and 1
        self.SR = np.linalg.pinv(
            np.eye(N_states) - discount_factor * T_prob
        )  # (N_states, N_states) matrix
        self.state_trajectory = np.zeros([N_timesteps])
    ...
    def reward_function_otheragents(
        self, xloc_neighbors, yloc_neighbors, xloc_self, yloc_self, edge_size
    ):
        """
        Parameters
        ----------
        ...
        for k in range(N_neighbors):
            ...
            reward_map[
                row + 0, col + 0
            ] = 1  # give the neighbor some personal space
...
class BCCH_species(object):
    def __init__(self, T_prob, N_states, N_timesteps, discount_factor):
        ...
        self.phi_neighbors = np.zeros(
            [N_states, 1]
        )  # in which locations do I see a neighboring agent?
    ...
    def reward_function_otheragents(
        ...
        for k in range(N_neighbors):
            ...
            reward_map[
                row + 0, col + 0
            ] = -2  # give the neighbor some personal space

        # make your current state neutral
        reward_map[
            yloc_self - 1 : yloc_self + 2, xloc_self - 1 : xloc_self + 2
        ] = 0
        reward_map[yloc_self, xloc_self] = 0
    ...
    def policy(self, prev_state, value, T_eligible, phi_agents):
        ...
        eligible_states_id = np.nonzero(
            T_eligible[:, prev_state] * np.logical_not(phi_agents.flatten())
        )[
            0
        ]  # state IDs of eligible states
        value_eligible = value[
            eligible_states_id
        ].flatten()  # value of eligible states plus some noise
        next_state = eligible_states_id[
