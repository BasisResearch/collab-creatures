...
did_reach_food = (
    time_to_first_food_allsims < N_timesteps
)  # did this bird reach food in time? (N_sims, N_agents)
num_agents_did_reach_food = np.sum(
    did_reach_food, axis=1
)  # number of birds that reached food in time (N_sims, 1)

# number of agents that failed to reach food
failed_to_reach_food = (
    1 - did_reach_food
)  # did this bird reach food in time? (N_sims, N_agents)
num_agents_failed_reach_food = np.sum(
    failed_to_reach_food, axis=1
)  # number of birds that reached food in time (N_sims, 1)
...
pop_mean_time_to_first_food = np.mean(
    time_to_first_food_allsims, axis=1
)  # (N_sims,) mean across individuals in the population
...
# %%
dist_to_nearest_neighbor_time_mean = np.mean(
    dist_to_nearest_neighbor_allsims, axis=2
)
dist_to_nearest_neighbor_pop_mean = np.mean(
    dist_to_nearest_neighbor_allsims, axis=(1, 2)
)
dist_to_nearest_neighbor_pop_var = np.var(
    dist_to_nearest_neighbor_allsims, axis=(1, 2)
)

calories_acquired_per_unit_time = (
    np.sum(calories_acquired_allsims, axis=(2)) / N_timesteps
)
...
if do_plot_dist_to_neighbors:
    ...
    ax.set_title(
        "Mean distance to nearest neighbor (across population and time)"
    )
    fig.tight_layout()
    ...
    ax.hist(dist_to_nearest_neighbor_pop_mean)
    ax.set_title(
        "Variance distance to nearest neighbor (across population and time)"
    )
    fig.tight_layout()
...
if do_plot_calories:
    ...
    ax.set_title(
        "Variance calories acquired per unit time (across population)"
    )
    fig.tight_layout()
...
if do_plot_model_internals:
    ...
    value_otheragents_2d = np.reshape(
        value_otheragents, (edge_size, edge_size)
    )
    ...
    val_weighted_sum = agent.SR @ (
        c_food_self * w_food + c_otheragents * w_otheragents_1d
    )
    ...
    sns.heatmap(
        ax=ax,
        data=np.reshape(value, (edge_size, edge_size)),
        yticklabels=False,
        xticklabels=False,
        square=True,
        cbar=True,
...
if do_plot_visible_food:
    ...
    fig, ax = plt.subplots()
    sns.heatmap(
        ax=ax, data=phi_visible_mat, vmin=0, vmax=1, center=0, cbar=True
    )
    ax.plot(xloc_self, yloc_self, ".")
...
if plotValueFuncAtTime:
    ...
    for ai, agent in enumerate(list_agents):
        ...
        value_heatmap = np.reshape(
            list_agents[ai].value_trajectory[:, ti], [edge_size, edge_size]
        )
        sns.heatmap(
            ax=ax[r, c],
            data=value_heatmap,
